(function(){
  ...
  const VOICE_MODE_KEY="libcomlair-voice-mode-v1";
  function preferredMode(){
    try{return localStorage.getItem(VOICE_MODE_KEY)||""}catch(_){return ""}
  }
  function savePreferredMode(mode){
    try{localStorage.setItem(VOICE_MODE_KEY,mode)}catch(_){}
  }
  function fallbackEngine(){return window.LibcomlairFallbackVoice||null}
  ...
  function cancel(){
    ...
    try{const fb=fallbackEngine();if(fb&&typeof fb.stop==="function")fb.stop()}catch(_){}
    emit("cancelled");
  }
  ...
  function runWithFallback(text,options){
    ...
    function finishError(error){
      ...
      const fb=fallbackEngine();
      if(fb&&typeof fb.status==="function"&&fb.status().ready){
        finished=true;
        emit("fallback",{error:String(error||"native_failed"),attempts:[...attempts]});
        fb.speak(clean,{
          onstart:meta=>{
            savePreferredMode("fallback");
            emit("started",{voice:"Voix de secours française",fallback:true,attempts:[...attempts]});
            if(typeof opts.onstart==="function")opts.onstart({voice:"Voix de secours française",fallback:true,meta});
          },
          onend:meta=>{
            emit("ended",{voice:"Voix de secours française",fallback:true,attempts:[...attempts]});
            if(typeof opts.onend==="function")opts.onend({voice:"Voix de secours française",fallback:true,meta});
          },
          onerror:e=>{
            const err=String(e&&e.message||e||error||"fallback_failed");
            emit("error",{error:err,attempts:[...attempts],fallback:true});
            if(typeof opts.onerror==="function")opts.onerror({error:err,attempts:[...attempts],fallback:true});
          }
        }).catch(()=>{});
        return;
      }
  ...
  function speak(text,options){
    ...
    const fb=fallbackEngine();
    if(preferredMode()==="fallback"&&fb&&typeof fb.status==="function"&&fb.status().ready){
      fb.speak(clean,{
        ...(options||{}),
        onstart:meta=>{
          emit("started",{voice:"Voix de secours française",fallback:true});
          if(options&&typeof options.onstart==="function")options.onstart({voice:"Voix de secours française",fallback:true,meta});
        },
        onend:meta=>{
          emit("ended",{voice:"Voix de secours française",fallback:true});
          if(options&&typeof options.onend==="function")options.onend({voice:"Voix de secours française",fallback:true,meta});
        },
        onerror:err=>{
          emit("error",{error:String(err&&err.message||err||"fallback_failed"),fallback:true});
          if(options&&typeof options.onerror==="function")options.onerror(err);
        }
      }).catch(()=>{});
      return true;
    }
    if(!available&&fb&&fb.status&&fb.status().ready){
      fb.speak(clean,options||{}).catch(()=>{});
      return true;
    }
  ...
  function status(){
    return {
      available:available||!!window.LibcomlairFallbackVoice,
      recognitionActive,
