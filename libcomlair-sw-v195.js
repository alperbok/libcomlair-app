const CACHE="libcomlair-v195-shell";
const START="./test-v195-vision-app.html";
const ASSETS=[
  START,
  "./style.css",
...
self.addEventListener("install",event=>{
  ...
  event.waitUntil((async()=>{
    ...
    await cache.add(START);
    await Promise.allSettled(ASSETS.filter(x=>x!==START).map(x=>cache.add(x)));
  })());
});
...
self.addEventListener("fetch",event=>{
  ...
  event.respondWith((async()=>{
    try{
      ...
      return (await caches.match(event.request)) || (await caches.match(START));
    }
  })());
});