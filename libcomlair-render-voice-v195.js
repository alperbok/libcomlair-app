(function(){
  ...
  function prepare(){
    ...
    emit("warming");
    preparePromise=checkStatus().then(()=>{
      prepared=true;
      return true;
    }).catch(error=>{
      preparePromise=null;
  ...
  async function playBuffer(buffer,meta,opts,myGen){
    ...
    let decoded=meta&&meta.cacheKey?decodeCache.get(meta.cacheKey):null;
    if(!decoded){
      decoded=await ctx.decodeAudioData(buffer.slice(0));
      if(meta&&meta.cacheKey)decodeCache.set(meta.cacheKey,decoded);
    }
    if(myGen!==generation)throw new Error("Lecture annulée.");
  ...
  async function prefetch(text){
    ...
    const ctx=getAudioContext();
    if(ctx&&!decodeCache.has(clean)){
      try{
        const decoded=await ctx.decodeAudioData(meta.buffer.slice(0));
        decodeCache.set(clean,decoded);
      }catch(_){}
    }
  ...
  // Réveille Render dès l'ouverture de la page. Sur l'offre gratuite,
  // le premier démarrage peut prendre 50 secondes ou davantage.
  prepare().catch(()=>{});
})();