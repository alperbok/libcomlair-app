(function(){
  ...
  function run(){
    ...
    let knownIssues=[];
    try{
      knownIssues=knownIssuesEngine&&typeof knownIssuesEngine.detect==="function"?knownIssuesEngine.detect():[];
    }catch(_){knownIssues=["known-issues-engine-error"]}
    ...
    const result={ok:failed.length===0&&runtimeErrors.length===0,checks,failed,knownIssues:[...knownIssues],runtimeErrors:[...runtimeErrors],version:"v189-known-issues"};
    window.__libcomlairLastDiagnostic=result;

    const box=document.getElementById("systemDiagnosticResult");
    if(box){
      if(result.ok){
        box.textContent="✓ Diagnostic réussi : les "+checks.length+" contrôles sont opérationnels. Aucune panne connue détectée. Voix : "+voiceMode+".";
      }else{
        const parts=[];
