(function(){
  ...
  try{webSpeechFailed=sessionStorage.getItem("libcomlair-webspeech-failed-v181")==="1"}catch(_){}
    ...
    try{
      ...
      for(let i=0;i<chunks.length;i++){
        ...
        await playBlob(wav,myGeneration,()=>{
          ...
          started=true;
          lastMode="fallback";
          const meta={voice:"Voix de secours Piper",mode:"fallback",runtime:piperRuntime};
          emit("fallback-started",meta);
          if(typeof opts.onstart==="function")opts.onstart(meta);
        });
      }
      if(myGeneration!==generation)return;
      emit("fallback-ended",{mode:"fallback",voice:"Voix de secours Piper",runtime:piperRuntime});
      if(typeof opts.onend==="function")opts.onend({voice:"Voix de secours Piper",mode:"fallback",runtime:piperRuntime});
    }catch(error){
      if(myGeneration!==generation)return;
      const message=error&&error.message?error.message:String(error||"fallback_failed");
      emit("error",{mode:"fallback",error:message});
      if(typeof opts.onerror==="function")opts.onerror({error:message,mode:"fallback"});
    }
  }

  function markWebSpeechFailed(){
    webSpeechFailed=true;
    try{sessionStorage.setItem("libcomlair-webspeech-failed-v181","1")}catch(_){}
  }
    ...
    function tryNext(){
      ...
      try{
        ...
        u.onstart=()=>{
          ...
          lastMode="normal";
          const meta={voice:label,attempts:[...attempts],mode:"normal"};
          emit("started",meta);
          if(typeof opts.onstart==="function")opts.onstart(meta);
        };
        u.onend=()=>{
        ...
        timer=setTimeout(()=>{
          ...
          try{
            if(synth.speaking){
              finished=true;
              lastMode="normal";
              const meta={voice:label,attempts:[...attempts],mode:"normal",inferred:true};
              emit("started",meta);
              if(typeof opts.onstart==="function")opts.onstart(meta);
              return;
            }
