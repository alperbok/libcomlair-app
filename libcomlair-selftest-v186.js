(function(){
  ...
  function run(){
    ...
    if(voiceState&&voiceState.activeEngine==="mespeak")voiceMode="voix de secours meSpeak active";
    else if(voiceState&&voiceState.activeEngine==="web")voiceMode="voix normale active";
    ...
    else if(voiceState&&voiceState.lastOutcome&&voiceState.lastOutcome.ok===true&&voiceState.lastOutcome.engine==="web")voiceMode="voix normale validée";
    else if(voiceHadFailure)voiceMode="dernière lecture vocale en échec";

    const checks=[
      check("Voix",!!(voiceEngine&&voiceEngine.version==="v186"&&typeof voiceEngine.speak==="function"&&typeof voiceEngine.testDetailed==="function"&&!voiceHadFailure),voiceMode),
    ...
    const result={ok:failed.length===0&&runtimeErrors.length===0,checks,failed,runtimeErrors:[...runtimeErrors],version:"v186"};
