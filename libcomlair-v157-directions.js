...
const STATIC_VERSION="157";
const staticShardCache=new Map();
const CACHE_KEY="libcomlair-idfm-directions-v157";
...
async function fetchStaticPayloads(ids){
  const prefixes=[...new Set(ids.map(id=>String(id).padStart(3,"0").slice(0,3)))];
  const shards=await Promise.all(prefixes.map(fetchStaticShard));
  const byPrefix=new Map();
  prefixes.forEach((p,i)=>byPrefix.set(p,shards[i]));
  const payloads=[];
  for(const id of ids){
    const prefix=String(id).padStart(3,"0").slice(0,3);
