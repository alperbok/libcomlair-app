(function(){
  ...
  function run(){
    ...
    const voiceHadRealFailure=!!(voiceState&&voiceState.lastOutcome&&voiceState.lastOutcome.ok===false); const voiceMode=voiceState&&voiceState.activeEngine==="piper"?"voix de secours active":voiceState&&voiceState.activeEngine==="web"?"voix normale active":voiceHadRealFailure?"dernière lecture en échec — superviseur revenu à l’état prêt":"superviseur vocal prêt";
    const checks=[
      check("Voix",!!(voiceEngine&&voiceEngine.version==="v183"&&typeof voiceEngine.speak==="function"&&typeof voiceEngine.testDetailed==="function"&&!voiceHadRealFailure),voiceHadRealFailure?"dernier test vocal en échec":voiceMode),
